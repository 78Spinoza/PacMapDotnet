use ndarray::Array2;
use crate::pacman_core::{Configuration, fit_transform_deterministic, PairConfiguration, PacmapConfig, PairConfig, KnnConfig, GradientConfig, AdamConfig};
use crate::pairs::{get_knn_indices};
//#[cfg(feature = "use_hnsw")]
//use hnsw::Hnsw;  // TEMPORARY: Commented out until proper HNSW implementation
use ndarray::Array1;
use std::sync::atomic::{AtomicBool, Ordering};
use crate::ffi::safe_progress_report;
use crate::serialization::{TransformStats, custom_serialize_hnsw, serialize_hnsw_to_bytes};
...
/// Convert FFI Configuration to pacman-core PacmapConfig
fn convert_to_pacman_config(config: &Configuration, progress_callback: Option<crate::pacman_core::ProgressCallback>) -> PacmapConfig {
    ...
    PacmapConfig {
        n_dims: config.embedding_dimensions,
        n_neighbors,
        n_mn: (n_neighbors as f32 * config.mid_near_ratio) as usize,
        n_fp: (n_neighbors as f32 * config.far_pair_ratio) as usize,
        seed: config.random_state,
        n_iters: total_epochs,
        pair_config: PairConfig::default(),
        knn_config: KnnConfig::default(),
        gradient_config: GradientConfig {
            loss_type: "pacmap".to_string(),
            progress_callback: None, // We'll handle this separately to avoid clone issues
        },
        adam_config: AdamConfig {
            learning_rate: config.learning_rate,
        },
        progress_callback, // Enable professional progress reporting
        report_progress: true,                // Enable detailed progress during optimization
    }
}
...
pub fn fit_transform_hnsw_with_params(data: Array2<f64>, config: Configuration, force_exact_knn: bool, progress_callback: Option<&(dyn Fn(&str, usize, usize, f32, &str) + Send + Sync)>, custom_hnsw_params: Option<HnswParams>, autodetect_hnsw_params: bool) -> Result<(Array2<f64>, Option<HnswParams>), Box<dyn std::error::Error>> {
    ...
    let _report_progress = |phase: &str, current: usize, total: usize, percent: f32, message: &str| {
        if let Some(callback) = progress_callback {
    ...
    let updated_config = if use_hnsw {
        ...
        // REMOVED: Spammy progress callback - eliminates garbage output
        ...
        if autodetect_hnsw_params {
            ...
            match crate::recall_validation::validate_hnsw_quality_with_retry_and_params(data.view(), n_neighbors, seed, hnsw_params.ef_search, Some(hnsw_params.clone())) {
        ...
        let hnsw_pairs = match crate::pairs::compute_pairs_hnsw_to_per_point_with_params(data.view(), n_neighbors, seed, Some(hnsw_params.clone())) {
            ...
            Err(e) => {
                ...
                let pairs = crate::pairs::compute_pairs_hnsw_with_params(data.view(), n_neighbors, seed, Some(hnsw_params.clone()));
                pairs
            }
        };
    ...
    // Convert Configuration to PacmapConfig for deterministic core with proper callback
    let callback_for_core: Option<crate::pacman_core::ProgressCallback> = Some(Box::new(|phase: &str, current: usize, total: usize, percent: f32, message: &str| {
        // Forward pacman-core progress to our global callback system
        safe_progress_report(phase, current, total, percent, message);
    }));
    let pacman_config = convert_to_pacman_config(&updated_config, callback_for_core);
    let pacmap_result = fit_transform_deterministic(data_f32.view(), &pacman_config)
        .map_err(|e| format!("Deterministic PaCMAP fitting failed: {}", e))?;
    let embedding_f32 = pacmap_result.embedding;
...
pub fn fit_transform_normalized_with_progress_and_force_knn_with_hnsw(
    ...
    let _progress = |phase: &str, current: usize, total: usize, percent: f32, message: &str| {
    ...
    let _seed = config.random_state.unwrap_or(42);
    ...
    if !force_exact_knn {
        ...
        let _hnsw_message = format!("HNSW: M={}, ef_construction={}, ef_search={}",
    ...
    let mut model = PaCMAP {
        ...
        used_hnsw: !force_exact_knn, // Track whether HNSW was used during training
        hnsw_index_crc32: None,
        fitted_projections_crc32: None,
        embedding_centroid, // Store centroid for "No Man's Land" detection
        fitted_projections: embedding.clone(), // ALWAYS save exact fitted projections (2D embedded space for new data)
        embedding_hnsw_index: None, // Will be populated for neighborhood search in embedded space
        serialized_hnsw_index: None, // Will be populated for massive datasets
        original_data: Some(data.clone()), // Store original training data for transform support
    };

    // Note: Store transform data functionality removed - using fitted_projections field directly

    // Build HNSW index for embedded space neighborhood search
    if !force_exact_knn && n_samples >= 1000 {
        #[cfg(feature = "use_hnsw")]
        {
            vprint!("Building HNSW index for embedded space neighborhood search...");
            // progress("Building Indices", 95, 100, 95.0, "Building HNSW index for neighborhood search");

            // Build embedded space HNSW index for new data analysis
            let embedded_points = &model.fitted_projections; // 2D embedded space
            let (n_embedded_samples, _) = embedded_points.dim();
            let hnsw_params = &model.config.hnsw_params;

            const MASSIVE_DATASET_THRESHOLD: usize = 500_000; // 500k points

            if n_embedded_samples > MASSIVE_DATASET_THRESHOLD {
                // For massive datasets, serialize the embedded space index for fast loading
                vprint!("[Package] Massive dataset detected: serializing embedded space HNSW index");
                let max_layer = ((n_embedded_samples as f32).ln() / (hnsw_params.m as f32).ln()).ceil() as usize + 1;
                let max_layer = max_layer.min(32).max(4);

                // Create HNSW index for embedded space (2D projections)
                let embedding_index = custom_serialize_hnsw(&embedded_points, hnsw_params, max_layer)?;
                ...
                model.serialized_hnsw_index = Some(embedding_bytes);
                model.hnsw_index_crc32 = Some(embedding_crc);
                model.fitted_projections_crc32 = Some(fitted_projections_crc);

                vprint!("[OK] Embedded space HNSW index serialized for massive dataset ({} points) with CRC checksums", n_embedded_samples);
            } else {
                // For smaller datasets, build embedded space HNSW index directly
                vprint!("[BUILD] Building embedded space HNSW index for neighborhood search");
                // TODO: Build actual HNSW index for embedded space when space crate limitations are resolved
                model.config.used_hnsw = false; // Temporary: disabled due to space crate limitations
            }
        }
    }
...
pub fn transform_with_model(model: &mut PaCMAP, mut new_data: Array2<f64>) -> Result<Array2<f64>, Box<dyn std::error::Error>> {
    ...
    let initial_neighbors: Vec<Vec<usize>> = if needs_hnsw {
        #[cfg(feature = "use_hnsw")]
        {
            ...
            if let Some(ref hnsw_index) = model.embedding_hnsw_index {
                ...
                (0..n_samples).map(|i| {
                    ...
                    neighbors.into_iter().map(|n| n.index).collect::<Vec<usize>>()
                }).collect()
            } else {
...
pub extern "C" fn pacmap_distance_stats(
    ...
    let (m, p, mx) = compute_distance_stats(&emb_arr); // Use default seed for FFI consistency
    unsafe {
        *mean = m;
        *p95 = p;
        *max = mx;
...
fn find_neighbors_in_original_space(
    ...
    // TEMPORARY: HNSW disabled - always fall back to brute-force
    #[cfg(feature = "use_hnsw")]
    {
        vprint!("[SEARCH] Transform: HNSW temporarily disabled - using brute-force neighbor search ({} points)", n_orig);

        // Fall back to brute-force neighbor search
        use crate::pairs::get_knn_indices_bruteforce;
        let all_neighbors = get_knn_indices_bruteforce(original_data.view(), k);

        vprint!("[OK] Transform: Brute-force neighbor search completed");
        return Ok(all_neighbors);
    }
...
fn find_neighbors_in_embedding_space(
    ...
    #[cfg(feature = "use_hnsw")]
    {
        ...
        // TEMPORARY: HNSW disabled - use brute-force for embedding space
        vprint!("[TEMPORARY] Transform: HNSW disabled - using brute-force for embedding space ({} points)", n_fitted);

        // Use brute-force for embedding space (always safe for 2D)
        for i in 0..n_new {
            let mut distances: Vec<(usize, f64)> = fitted_projections
                .axis_iter(ndarray::Axis(0))
                .enumerate()
                .map(|(j, row)| {
                    let dist = euclidean_distance(new_projections.row(i), row);
                    (j, dist)
                })
                .collect();

            distances.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
            let neighbors: Vec<usize> = distances.into_iter().take(k).map(|(j, _)| j).collect();
            all_neighbors.push(neighbors);
        }

        vprint!("[OK] Brute-force embedding neighbor search completed");
        return Ok(all_neighbors);
    }
...
pub fn transform_with_stats(model: &mut PaCMAP, new_data: Array2<f64>) -> Result<Vec<TransformStats>, Box<dyn std::error::Error>> {
    ...
    // TODO: Implement proper HNSW index management
    if model.config.used_hnsw {
        #[cfg(feature = "use_hnsw")]
        {
            // model.ensure_embedding_hnsw_index()?; // TODO: Implement this method
            eprintln!("⚠️  HNSW index management not yet implemented");
        }
    }
        ...
        #[cfg(feature = "use_hnsw")]
        {
            ...
            if let Some(ref embedding_hnsw) = model.embedding_hnsw_index {
                ...
                (0..n_samples).map(|i| {
                    ...
                    TransformStats {
                        transform_time_ms: 0, // TODO: Implement timing
                        neighbors_found: k_neighbors.len(),
                        avg_distance: mean_distance_to_k_neighbors,
                        coordinates: Some(point_coords.to_vec()),
...
// Include the model persistence tests
#[cfg(test)]
mod test_model_persistence;
