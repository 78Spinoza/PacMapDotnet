    ...
    Zip::from(y.view_mut())
        ...
        .par_for_each(|y, &grad, m, v| {
            ...
            *y -= param_update;
            ...
            let _ = m_update_abs.max(&mut max_m_update);
            let _ = v_update_abs.max(&mut max_v_update);
            let _ = param_update_abs.max(&mut max_param_update);
...
pub fn reset_adam_state(m: ArrayViewMut2<f32>, v: ArrayViewMut2<f32>) {
    m.fill(0.0);
    v.fill(0.0);
}
