...
use std::os::raw::{c_char, c_int, c_double, c_uchar, c_ulong};
use std::sync::Mutex;
use std::panic;
use lazy_static::lazy_static;
use ndarray::Array2;
use crate::pacman_core::Configuration;
...
/// Safe progress callback function type for C# integration
/// Uses the proven ptr+len pattern to avoid memory management issues
pub type ProgressCallback = extern "C" fn(
    user_data: usize,           // User-provided data pointer (for C# state) - use usize for thread safety
    data: *const c_uchar,       // UTF-8 encoded message bytes
    len: c_ulong,               // Length of the message in bytes
);

lazy_static! {
    static ref PROGRESS_CALLBACK: Mutex<Option<(ProgressCallback, usize)>> = Mutex::new(None);
}

/// Register progress callback with the Rust library
/// Thread-safe and panic-safe
#[no_mangle]
pub extern "C" fn pacmap_register_progress_callback(
    callback: Option<ProgressCallback>,
    user_data: usize,
) {
    let mut cb_slot = PROGRESS_CALLBACK.lock().unwrap();
    if let Some(cb_fn) = callback {
        *cb_slot = Some((cb_fn, user_data));
    } else {
        *cb_slot = None;
    }
}

/// Unregister progress callback - convenience function
#[no_mangle]
pub extern "C" fn pacmap_unregister_progress_callback() {
    let mut cb_slot = PROGRESS_CALLBACK.lock().unwrap();
    *cb_slot = None;
}

/// Safe progress reporting function - avoids all memory management issues
fn report_progress_safe(message: &str) {
    let callback_slot = PROGRESS_CALLBACK.lock().unwrap();
    if let Some((callback, user_data)) = *callback_slot {
        // Convert message to UTF-8 bytes
        let message_bytes = message.as_bytes();

        // Call callback with panic protection
        let _ = panic::catch_unwind(|| {
            callback(user_data, message_bytes.as_ptr(), message_bytes.len() as c_ulong);
        });
    }
}

/// Safe progress reporting function - eliminates the memory corruption bug
pub fn safe_progress_report(phase: &str, _current: usize, _total: usize, percent: f32, message: &str) {
    let callback_slot = PROGRESS_CALLBACK.lock().unwrap();
    if let Some((callback, user_data)) = *callback_slot {
        // Format the progress message
        let full_message = if message.is_empty() {
            format!("[{}] ({:.1}%)", phase, percent)
        } else {
            format!("[{}] {} ({:.1}%)", phase, message, percent)
        };
...
impl PacmapConfig {
    ...
    fn to_hnsw_params(&self, n_samples: usize, n_features: usize) -> HnswParams {
        if self.hnsw_config.auto_scale {
            ...
            // TEMPORARY: Use existing auto_scale function (density_scaling ignored until HNSW migration complete)
            HnswParams::auto_scale(n_samples, n_features, 15)
        } else {
            HnswParams {
                ...
                // TEMPORARY: density_scaling field removed until HNSW migration complete
...
// OLD CORRUPTED MACRO REMOVED - replaced with safe callback system above
...
pub extern "C" fn pacmap_fit_transform_enhanced(
    ...
    safe_progress_report("Initializing", 0, 100, 0.0, "Preparing dataset for PacMAP fitting");
    ...
    if !config.force_exact_knn {
        ...
        safe_progress_report("HNSW Config", 10, 100, 10.0, &hnsw_message);
    } else {
        safe_progress_report("KNN Config", 10, 100, 10.0, "Exact KNN requested - HNSW disabled for precision");
    }
    ...
    safe_progress_report("Normalizing", 20, 100, 20.0, "Applying data normalization");

    // Perform fitting with progress callback and force_exact_knn control
    // DISABLED: Memory corruption bug in callback closure
    // The Box::new callback approach causes use-after-free when CString objects are dropped
    // while C# still has pointers to freed memory
    let rust_progress_callback = None; // Temporarily disabled to fix memory corruption
    ...
    safe_progress_report("FFI Debug", 3, 100, 3.0, &ffi_debug_msg);
    ...
    safe_progress_report("Debug Start", 4, 100, 4.0, &debug_msg);

    // DEBUG: Log HNSW config values to check struct alignment
    let hnsw_debug_msg = format!(" FFI DEBUG: HNSW config - auto_scale: {}, use_case: {}, m: {}, ef_construction: {}, ef_search: {}, memory_limit_mb: {}, density_scaling: {}, autodetect_hnsw_params: {}",
        config.hnsw_config.auto_scale, config.hnsw_config.use_case, config.hnsw_config.m, config.hnsw_config.ef_construction, config.hnsw_config.ef_search, config.hnsw_config.memory_limit_mb, config.hnsw_config.density_scaling, config.hnsw_config.autodetect_hnsw_params);
    safe_progress_report("Debug HNSW", 5, 100, 5.0, &hnsw_debug_msg);
    ...
    match fit_transform_normalized_with_progress_and_force_knn_with_hnsw(
        ...
        Ok((result_embedding, model)) => {
            // REMOVED: Spammy progress callbacks causing memory corruption
            ...
            // Report completion
            safe_progress_report("Complete", 100, 100, 100.0, "PacMAP fitting completed successfully");
        ...
        Err(e) => {
            ...
            safe_progress_report("Error Details", 1, 100, 1.0, &error_msg);
            ...
            safe_progress_report(error_type, 0, 100, 0.0, &format!("PacMAP fitting failed: {}", e));
            std::ptr::null_mut()
        }
    }
}
...
pub extern "C" fn pacmap_transform(
    ...
    safe_progress_report("Transform", 0, 100, 0.0, "Transforming new data using existing model");

    // Perform transformation
    match transform_with_model(model, data_arr) {
        Ok(result_embedding) => {
            safe_progress_report("Copying", 50, 100, 50.0, "Copying transformation results");
            ...
            safe_progress_report("Complete", 100, 100, 100.0, "Transform completed successfully");
            0 // Success
        }
        Err(e) => {
            // Detailed error reporting for transform failures
            let error_msg = format!("Transform failed: {}", e);
            safe_progress_report("Transform Error", 0, 100, 0.0, &error_msg);
